...
独立测试：打开 `http://127.0.0.1:4173/?encounterDebug=fast`，开发控制台执行 `debugOpenIntentJournal('giant_whale_shadow')` 可直接试读线索；也支持 `debugSetVoyageIntent(id)`、`debugClearVoyageIntent()`、`getCurrentVoyageIntentDebug()`。测试记录只写 `tiny-tides-encounters-debug-v1`，正式构建删除这些入口。正常预览需真实见闻完成才生成日志；GM 演出不产生正式记录。
