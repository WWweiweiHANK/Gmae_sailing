...
船名中文最多 6 字、英文数字最多 12 字，混合名称使用宽度预算并按实测文字宽度缩放。确认后应用名称；皮肤和已拥有徽章点击即应用。统一存档保存在当前来源的 localStorage `tiny-tides-game-v1`（内部版本为 7），保留旧颜色数据用于兼容，但游戏外观由 skinId 与 accessories 决定。浏览器预览和 Tauri 各自持久保存，不自动跨来源同步。非法字段会回退到默认值；无法解析或版本不兼容的存档禁止覆盖，保存失败会在面板提示。
...
版本 5 及更早的存档默认使用经典款；旧颜色归属和颜色值继续存储，仅作历史兼容。没有擅自返还或扣除过去花费的航行值。已有日志、纪念物和徽章均保留。当前存档版本为 7；回退到旧代码前先备份统一存档，旧版不识别新版本。刷新 http://127.0.0.1:4173/ 后进入「我的船 → 皮肤」即可测试。

## 七个通用安装位置 · 船用装饰

进入「我的船 → 装饰」，选择安装位置，再点击饰品即可保存。按参考图制作 36 款低多边形挂件、立体徽章、摆件与可选组件，另有原装烟囱及奶油/木色/海蓝三款铭牌。当前全部免费试装，不扣航行值，不假冒见闻解锁记录。每个位置一次安装一件；除铭牌外均可选择“不安装”，位置仍保留。前甲板与顶板可同时放置不同摆件。
...
2026-09-20 已按 `history/references/boat-assembly-v01.png` 重建实际模型：更饱满的船身、倒角屋顶与窗框、锥形方烟囱、连续弯曲的海豚/鲸鱼身体、真实分段救生圈、缩短挂绳、放大的灯塔及展开尖叶绿植；其他摆件共用的轮廓和几何也同步替换。已装备的饰品自动显示新模型，不需要重新购买或清空存档；保持版本 7。要对照参考图，可在经典款上安装红旗、灯塔、原装烟囱、救生圈、奶油铭牌、海豚挂件、绿植箱。前甲板/顶板按物件包围盒限宽限深，挂件限定总下垂长度。
...
第一次打开从第一页开始，此后优先首条未读所在双页，无未读时打开最后一页；打开时清除已有未读，阅读过程中新增记录只更新轻量提示，不强行跳页。旧日志保留当时船名。该阶段曾将统一存档升级为版本 5（当前为版本 7），增加 `journalEntries` 和 `journalState`，保留航行值、外观、徽章、纪念物及见闻历史；旧历史缺少完整逐次内容，因此不会追溯编造旧日志。浏览器和 EXE 存档仍按来源隔离。回退到旧版前请备份统一存档；旧版本不识别版本 5。
