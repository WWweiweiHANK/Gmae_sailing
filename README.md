...
进入「我的船 → 装饰」，选择安装位置，再点击饰品即可保存。当前按用户选定保留 18 件：C1、B1–B9、D1、D5、A1、A2、A4、A5、A6、A9；另有原装烟囱及奶油/木色/海蓝三款铭牌作为基础组件。旧装备中被移除的装饰回到对应安装位默认值，保留航行进度、皮肤与船名。当前仍免费试装，事件奖励映射等待“未匹配项如何处理”的明确选择，不假冒已完成解锁接入。每个位置一次安装一件；除铭牌外均可选择“不安装”，位置仍保留。
