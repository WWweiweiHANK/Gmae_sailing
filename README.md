...
顶部是船名与编辑笔，右上是可点击的「海里」说明；中央保留航行中的真实场景，滚轮可在 0.84–1.12 倍观察距离内轻微缩放。底部只有「船体／装饰」两个一级页签：船体提供九款免费整船皮肤，装饰可选七个安装位置或航行徽章，不恢复已取消的颜色购买。10 个体验徽章保留原有解锁规则，与船名共用双侧铭牌 CanvasTexture；不受救生圈位置的立体装饰徽章影响。

点击编辑笔在标题原位改名：Enter 或点击其他区域保存，Escape 取消；同步顶部、3D 铭牌与日志封面，不改旧日志记录中的历史船名。中文最多 6 字、英文数字最多 12 字，混合名称使用宽度预算。皮肤和已拥有徽章点击即应用。统一存档保存在当前来源的 localStorage `tiny-tides-game-v1`（内部版本仍为 8），保留旧颜色兼容字段；浏览器和 Tauri 各自持久保存，不自动跨来源同步。非法字段回退默认值；无法解析或版本不兼容的存档禁止覆盖，保存失败在面板提示。

关闭或 Escape 平滑返回进入前的相机位置和观察目标，小船从当前航行位置继续，不倒退航线；打开日志时 Escape 先合上日志。进入时清除残余视角惯性，返回后不发生角度漂移。如果此前手动暂停了世界，展示和返回后都保持暂停。展示中不会保存临时近景为正常观察角度。支持减少动态效果设置，沿用原天气和徽章逻辑。
...
船只默认缩小到原尺寸的 80%。进入「我的船 → 船体」，底部「船只大小」滑杆可在 50%–100% 调整，100% 恢复本轮修改前大小；切换船型保留比例，饰品、铭牌、灯位和新尾流同步缩放。设置保存于当前来源 localStorage 的 `tiny-tides-boat-size-v1`，网页与桌面分别保留。开发默认值和范围在 `boat-model.mjs` 的 `BOAT_SIZE`，界面范围在 `template.html`。
...
版本 5 及更早的存档默认使用经典款；旧颜色归属和颜色值继续存储，仅作历史兼容。没有擅自返还或扣除过去花费的航行值。已有日志、纪念物和徽章均保留。当前存档版本为 8；回退到旧代码前先备份统一存档，旧版不识别新版本。刷新 http://127.0.0.1:4173/ 后进入「我的船 → 船体」即可测试。
...
进入「我的船 → 装饰」，选择安装位置，再点击饰品即可保存。当前按用户选定保留 18 件：C1、B1–B9、D1、D5、A1、A2、A4、A5、A6、A9；另有原装烟囱及奶油/木色/海蓝三款铭牌作为基础组件。旧装备中被移除的装饰回到对应安装位默认值，保留航行进度、皮肤与船名。9 件装饰由对应见闻正常完成后获得；未匹配事件的 9 件保留饰品及基础组件默认可用。GM 构建可全部试装，但不会授予纪念物或写入见闻日志。版本 7 及更早存档中正在装备的保留饰品继承为已拥有；历史纪念物与日志保留。每个位置一次安装一件；除铭牌外均可选择“不安装”，位置仍保留。
...
刷新本地预览，点击小船进入「我的船」，点击模型区域左下、装扮面板上方的闭合小书。封面显示 VOYAGE LOG、当前船名与已佩戴徽章，未读用浅色书签提示，Hover 轻浮 4px。入口和展开日志复用同一封面 DOM：移动放大再开封面，合上后回到原位置。小船继续航行，镜头后退移向左边；窄屏为上船下书。开书约 1.15 秒、合书约 0.9 秒、翻页约 0.76 秒。纸页边缘、底部箭头、方向键和 PageUp/PageDown 可翻页；Home/End 到首末页，滚轮冷却 950 毫秒。Escape 先合书再退出我的船，支持减少动画，无翻页声音。
...
第一次打开从第一页开始，此后优先首条未读所在双页，无未读时打开最后一页；打开时清除已有未读，阅读过程中新增记录只更新轻量提示，不强行跳页。旧日志保留当时船名。该阶段曾将统一存档升级为版本 5（当前为版本 8），增加 `journalEntries` 和 `journalState`，保留航行值、外观、徽章、纪念物及见闻历史；旧历史缺少完整逐次内容，因此不会追溯编造旧日志。浏览器和 EXE 存档仍按来源隔离。回退到旧版前请备份统一存档；旧版本不识别版本 5。
...
玩家界面显示「≋ 128 海里」，右上胶囊点击展开轻量说明，点空白处或 Escape 关闭。正常规则为「1 分钟航行 = 1 海里」；当前整船皮肤免费、纪念装饰随见闻解锁，因此说明不会声称存在已取消的颜色商店。内部 sailingData.points、计时、余额和旧消费记录不变。
...
继续使用根目录源码开发；`npm run build` 后刷新 http://127.0.0.1:4173/ 即可查看未打包效果。沿用现有「我的船」装饰与日志界面，无额外任务领取。普通海豚也由同一个 EncounterDirector 调度，旧的独立循环不再驱动实际场景。
...
- 只在场景实际绘制到可见阶段后发现，正常完成后授予一件对应装饰；中断、未见到和 GM 演出不发奖。无需点击领取、不扣航行值、不会重复发放，不再同时授予海豚或鲸的航行徽章；过去已拥有的徽章保留。粉色来客具有固定 visitor ID、不同体型比例和绕船头的伴游路径。
