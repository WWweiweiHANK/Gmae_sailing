...
底部现在是皮肤、装饰、徽章、船名四个页签。九款整船皮肤全部直接可切换；不再提供部位调色或颜色购买。另有 10 个体验徽章，分为已拥有、已知未解锁和未发现三类。体验徽章和船名绘制在船体两侧的固定铭牌位置，共用一张 CanvasTexture，随不同船体宽度重新定位；不受救生圈位置的装饰徽章影响。

船名中文最多 6 字、英文数字最多 12 字，混合名称使用宽度预算并按实测文字宽度缩放。确认后应用名称；皮肤和已拥有徽章点击即应用。统一存档保存在当前来源的 localStorage `tiny-tides-game-v1`（内部版本为 8），保留旧颜色数据用于兼容，但游戏外观由 skinId 与 accessories 决定。浏览器预览和 Tauri 各自持久保存，不自动跨来源同步。非法字段会回退到默认值；无法解析或版本不兼容的存档禁止覆盖，保存失败会在面板提示。
...
版本 5 及更早的存档默认使用经典款；旧颜色归属和颜色值继续存储，仅作历史兼容。没有擅自返还或扣除过去花费的航行值。已有日志、纪念物和徽章均保留。当前存档版本为 8；回退到旧代码前先备份统一存档，旧版不识别新版本。刷新 http://127.0.0.1:4173/ 后进入「我的船 → 皮肤」即可测试。
