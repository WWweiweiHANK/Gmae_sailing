...
当前开发版本含「我的船」、叙事日志与航行意图，使用 `npm run preview:gm` 保留 GM 工具预览。本次桌面交付为 `outputs/Tiny-Tides-GM-2026-09-20.exe`，旧 EXE 与安装包单独保留；源码构建与交付包独立维护。实测与未测项见 [TEST_RESULTS.md](TEST_RESULTS.md)。
...
点击编辑笔在标题原位改名：Enter 或点击其他区域保存，Escape 取消；同步顶部、3D 铭牌与日志封面，不改旧日志记录中的历史船名。中文最多 6 字、英文数字最多 12 字，混合名称使用宽度预算。皮肤和已拥有徽章点击即应用。统一存档保存在当前来源的 localStorage `tiny-tides-game-v1`（内部版本为 9），保留旧颜色兼容字段；浏览器和 Tauri 各自持久保存，不自动跨来源同步。非法字段回退默认值；无法解析或版本不兼容的存档禁止覆盖，保存失败在面板提示。
...
版本 5 及更早的存档默认使用经典款；旧颜色归属和颜色值继续存储，仅作历史兼容。没有擅自返还或扣除过去花费的航行值。已有日志、纪念物和徽章均保留。当前存档版本为 9；回退到旧代码前先备份统一存档，旧版不识别新版本。刷新 http://127.0.0.1:4173/ 后进入「我的船 → 船体」即可测试。
...
进入「我的船 → 装饰」，选择安装位置，再点击饰品即可保存。当前按用户选定保留 18 件：C1、B1–B9、D1、D5、A1、A2、A4、A5、A6、A9；另有原装烟囱及奶油/木色/海蓝三款铭牌作为基础组件。旧装备中被移除的装饰回到对应安装位默认值，保留航行进度、皮肤与船名。9 件装饰由对应见闻正常完成后获得；未匹配事件的 9 件保留饰品及基础组件默认可用。GM 构建可全部试装；仅试装不授予纪念物，GM 事件完整结束则正常记录和解锁。版本 7 及更早存档中正在装备的保留饰品继承为已拥有；历史纪念物与日志保留。每个位置一次安装一件；除铭牌外均可选择“不安装”，位置仍保留。
...
完整日志第一次打开从第一页开始，此后优先首条未读所在双页，无未读时打开最后一页。只标记实际打开双页中的普通记录；线索页在选择或主动合书时标记已读。阅读中新增记录只更新提示，不强行跳页。桌面小册直接打开最新未读双页。旧日志保留当时船名。该阶段曾将统一存档升级为版本 5（当前为版本 9），增加 `journalEntries` 和 `journalState`，保留航行值、外观、徽章、纪念物及见闻历史；旧历史缺少完整逐次内容，因此不会追溯编造旧日志。浏览器和 EXE 存档仍按来源隔离。
...
线索日志的纸页末尾出现“我想，接下来……”和两至三句淡墨候选。Hover 或键盘聚焦时显示素描批注；点击后约 1.05 秒写入正文、合书返回航行。每篇只能写一次，记录与意图原子保存；保存失败仍可重试。“也许以后再想。”同样返回航行，保持当前意图，之后仍能在完整日志里选择。短窗口长页可在纸页内滚动。

支持随海而行、追随深海踪迹、跟随迁徙鸟群、寻找灯光、等待星夜、顺着寒冷海流，以及粉色海豚再次来访后的跟随选择。普通鱼群、普通海豚、安静一天和第一次粉色海豚只记录；鲸影、鲸鱼浮出、迁徙、灯塔、流星、浮冰可提供不同线索句。

`voyage-intents.mjs` 集中配置时长、文案、倾向和回应。每次随机持续 20–45 分钟有效航行，最后两分钟权重缓慢回落；隐藏、暂停、展示和关闭应用时不推进，不补算离线时间。仅保留一个意图，新选择替换旧意图；过期或真实事件回应后安静回到自由航行。EncounterDirector 抽选时才读取权重，保留天气、前置、互斥、冷却和安静期，不保证或立即召唤目标。等待星夜只在原本适合极光的环境抽选中增加倾向。

回应日志保存来源意图和日志 ID；追鲸与不追鲸有不同续文。原一对一见闻纪念物规则保留，不增加奖励等级、任务或未实现事件。当前意图仅在最新页角落用淡铅笔字表示。

未读记录出现时，海洋旁左下角显示同一本迷你日志，400ms 轻浮淡入、不抢焦点；普通浅色书签，线索蓝色书签。没有未读时完全隐藏。合书未选也会标记当前线索为已读，因此桌面小册消失，但“我的船”里的完整日志入口一直保留。
