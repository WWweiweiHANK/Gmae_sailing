...
test('new saves own three colors, old equipped colors migrate without losing sailing or appearance',()=>{
 ...
 const saved=createGameSave(storage).read();assert.equal(saved.version,8);assert.ok(saved.ownedColors.includes('mint'));assert.ok(saved.ownedColors.includes('forest'));assert.equal(saved.shipCustomization.name,'晚风号');assert.equal(saved.sailingData.points,86);
});
