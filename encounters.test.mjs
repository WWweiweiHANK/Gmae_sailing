...
test('souvenirs unlock once and old saves retain all existing progression',()=>{
 ...
 storage.setItem(GAME_SAVE_KEY,JSON.stringify({version:3,shipCustomization:{name:'晚风号',hullColor:'#eddb9b'},sailingData:{points:85,totalSailingSeconds:500},ownedColors:['yellow']}));const migrated=createGameSave(storage).read();assert.equal(migrated.version,10);assert.equal(migrated.sailingData.points,85);assert.equal(migrated.shipCustomization.name,'晚风号');assert.deepEqual(migrated.encounterHistory,{});
});
