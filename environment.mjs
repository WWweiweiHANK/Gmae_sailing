...
export function createEnvironment(random=Math.random,initial={}){
 ...
 function updateAurora(){const suitable=!initial.disableAurora&&period==='night'&&(weather==='clear'||weather==='overcast');if(suitable&&!eligible)auroraAt=random()<Math.min(.9,C.auroraChance*(initial.intentWeight?.()??1))?time+range(C.auroraDelay):Infinity;if(!suitable)auroraAt=Infinity;eligible=suitable;}
