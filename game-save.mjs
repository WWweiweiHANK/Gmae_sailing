...
export function createGameSave(storage,{key=GAME_SAVE_KEY,onError=()=>{},migrateLegacy=true}={}){
 ...
 try{
  ...
  if(loaded&&![1,2,3,4,5,6].includes(loaded.version)){readOnly=true;onError('存档版本不兼容，已保护原存档；本次修改不会保存。');}
 ...
 function normalize(value){
  ...
  return {...sanitizeEncounters(value),...sanitizeJournal(value),version:6,shipCustomization:ship,sailingData:sanitizeSailing(value.sailingData),ownedColors:ownedColorIds(value.ownedColors),colorHintSeen:value.colorHintSeen===true,ownedBadges:records,
 ...
 if(legacy||loaded?.version<6)save({});
