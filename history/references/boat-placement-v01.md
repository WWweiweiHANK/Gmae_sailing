...
2026-09-20 后续确认：未匹配饰品默认可用，未匹配事件暂不发饰品。已按保留的 18 件继续重建实际模型；当前对照页见 `../../qa/fidelity-review.html`，完整映射见 README。下述早先“待明确”是选图时的历史状态；生成候选图仍为 EXPLORE，未替换为实际渲染或自动升级为最终美术标准。
