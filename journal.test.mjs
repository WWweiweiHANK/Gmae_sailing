...
test('journal bookmarks and deduplicated annotations survive unrelated saves',()=>{
 const {journal:j,store,bus,storage}=setup();bus.emitWorldEvent('encounter_completed',event());const id=j.entries()[0].id;
 j.remember(6,'voyage');j.annotate({sourceJournalEntryId:id,slot:'charm',item:'pink-dolphin',text:'挂上了粉色海豚。'});
 j.annotate({sourceJournalEntryId:id,slot:'charm',item:'pink-dolphin',text:'挂上了粉色海豚。'});
 store.save({sailingData:{points:9}});j.markReadIds([id],true);
 const saved=createGameSave(storage).read();assert.equal(saved.journalAnnotations.length,1);assert.equal(saved.journalUiState.lastOpenedPage,6);assert.deepEqual(saved.journalUiState.unreadEntryIds,[]);assert.equal(saved.sailingData.points,9);
});
