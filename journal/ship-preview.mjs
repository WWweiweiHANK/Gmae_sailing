...
export function createShipPreview(renderer,ship){
 ...
 const pixels=new Uint8Array(size*size*4),frame=new ImageData(size,size),clear=new THREE.Color();let copy=null,canvas=null,last=0,yaw=.65,pitch=.37,drag=null,dirty=true,renderedFrames=0;
 function rebuild(){if(copy)scene.remove(copy);copy=clonePreviewShip(ship);scene.add(copy);dirty=true;}
 function bind(next){canvas=next;drag=null;if(!next)return;dirty=true;next.width=next.height=size;next.getContext('2d').putImageData(frame,0,0);next.tabIndex=0;next.setAttribute('aria-label','小船记录模型，拖动或使用方向键旋转');
  next.onpointerdown=e=>{if(e.button!==0)return;drag={id:e.pointerId,x:e.clientX,y:e.clientY};next.setPointerCapture(e.pointerId);};
  next.onpointermove=e=>{if(!drag||drag.id!==e.pointerId)return;yaw-=(e.clientX-drag.x)*.012;pitch=THREE.MathUtils.clamp(pitch+(e.clientY-drag.y)*.008,.12,.8);drag.x=e.clientX;drag.y=e.clientY;dirty=true;};
  next.onpointerup=next.onpointercancel=()=>{drag=null;};
  next.onkeydown=e=>{if(!e.key.startsWith('Arrow'))return;e.preventDefault();e.stopPropagation();yaw+=e.key==='ArrowLeft'?.15:e.key==='ArrowRight'?-.15:0;pitch=THREE.MathUtils.clamp(pitch+(e.key==='ArrowUp'?.08:e.key==='ArrowDown'?-.08:0),.12,.8);dirty=true;};
 }
 rebuild();
 return {rebuild,bind,render(now){if(!canvas?.isConnected||now-last<(dirty?1000/60-.5:500))return;last=now;dirty=false;renderedFrames++;camera.position.set(Math.sin(yaw)*6.4,1+Math.sin(pitch)*6.4,Math.cos(yaw)*6.4);camera.lookAt(0,1,0);
 ...
 },dispose(){bind(null);target.dispose();scene.clear();},snapshot:()=>({yaw,pitch,size,renderedFrames,active:!!canvas?.isConnected})};
}
