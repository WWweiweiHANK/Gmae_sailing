...
(async()=>{
 ...
 try{
  ...
  for(const scenario of ['day','storm','dusk','night','dawn']){
   ...
   await page.locator('#journal-open').click();await page.waitForTimeout(1200);
  ...
  await page.locator('#journal-next').click();await page.waitForFunction(()=>document.querySelector('#journal').dataset.state==='open');await page.locator('.left-page [data-skin="gentle"]').click();assert.equal((await state()).boatSize,.75);
  await page.setViewportSize({width:560,height:540});await page.screenshot({path:'qa/boat-size-control.png'});assert.equal(await page.locator('.left-page [data-skin]').count(),9);
