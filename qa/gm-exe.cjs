...
(async()=>{
 ...
 const child=spawn(exe,[],{windowsHide:true,env:{...process.env,WEBVIEW2_USER_DATA_FOLDER:profile,WEBVIEW2_ADDITIONAL_BROWSER_ARGUMENTS:'--remote-debugging-port=9223'}});let browser,page;
 const errors=[],checks=[];child.on('error',error=>errors.push(error.message));
 try{
  for(let attempt=0;attempt<30;attempt++){try{browser=await chromium.connectOverCDP('http://127.0.0.1:9223');break;}catch{await new Promise(r=>setTimeout(r,1000));}}
  ...
  page.on('pageerror',e=>errors.push(e.message));await context.addInitScript(()=>{window.oceanTools={};Object.defineProperty(document,'modelContext',{value:{registerTool:t=>window.oceanTools[t.name]=t}});});
  ...
  await page.locator('#journal-open').click();await page.waitForTimeout(1300);assert.equal(await page.locator('.left-page .intent-sentence').count(),3);await page.screenshot({path:'qa/gm-exe-journal.png'});
  await page.locator('.left-page [data-intent="follow_deep_creature"]').click();await page.waitForFunction(()=>document.body.dataset.showcase==='false');assert.equal((await save()).voyageIntentState.currentIntentId,'follow_deep_creature');
  ...
  await page.locator('#scene').focus();await page.keyboard.press('Enter');await page.waitForTimeout(1100);await page.locator('[data-skin="gentle"]').click();assert.equal((await state()).boatSize,.75);await page.keyboard.press('Escape');await page.waitForFunction(()=>document.body.dataset.showcase==='false');
