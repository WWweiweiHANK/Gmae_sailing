...
(async()=>{
 ...
 try{
  ...
  await page.locator('#journal-close').click();await page.waitForTimeout(850);
  ...
  assert.match(await page.locator('#journal-unread').textContent(),/10/);
  ...
  assert.equal(await page.locator('#journal-unread').textContent(),'');await page.screenshot({path:'qa/journal-wide.png'});
  ...
  await page.locator('.journal-book').hover();await page.mouse.wheel(0,140);await page.mouse.wheel(0,140);await page.mouse.wheel(0,140);await page.waitForTimeout(850);assert.match(await page.locator('#journal-page-number').textContent(),/^3 — 4/);
  await page.keyboard.press('Escape');await page.waitForTimeout(850);assert.equal(await page.locator('#journal').isVisible(),false);assert.equal(await page.locator('#ship-panel').isVisible(),true);assert.equal(await page.locator('#journal-open').evaluate(el=>document.activeElement===el),true);
  ...
  await page.keyboard.press('Escape');await page.waitForTimeout(180);assert.equal(await page.locator('#journal').isVisible(),false);
