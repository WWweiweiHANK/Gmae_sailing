...
export function createShipShowcase({canvas,camera,controls,ship,customization,onEnter,onLeave}){
 ...
 function open(){if(phase!=='sailing')return;previousFocus=document.activeElement;saved={position:camera.position.clone(),target:controls.target.clone(),enabled:controls.enabled};heading=ship.rotation.y;yaw=0;pitch=0;vYaw=0;vPitch=0;phase='opening';progress=0;controls.enabled=false;onEnter();document.body.dataset.showcase='true';dialog.hidden=false;dialog.classList.remove('leaving');nameInput.value=customization.data.name;notice.textContent='外观会自动保存在本机';tab('colors');refresh();document.querySelector('#ship-close').focus();}
