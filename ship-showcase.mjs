...
export function createShipShowcase({canvas,camera,controls,ship,customization,colors,badges,journal,onColorChange,onEnter,onLeave}){
 ...
 const journalView=createJournalView({journal,customization,canOpen:()=>phase==='open',onMode:()=>{drag=null;pressed=null;vYaw=0;vPitch=0;dismissPurchase();}});
 ...
 function close(){if(phase==='sailing'||phase==='closing')return;journalView.close(true);acknowledgeBadges();dismissPurchase();clearTimeout(noticeTimer);phase='closing';drag=null;pressed=null;vYaw=0;vPitch=0;dialog.classList.add('leaving');}
