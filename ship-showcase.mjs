...
export function createShipShowcase({canvas,camera,controls,ship,customization,skins,accessories,badges,journal,onEnter,onLeave}){
 ...
 document.querySelector('#decoration-category').addEventListener('change',e=>{acknowledgeBadges();const showBadges=e.target.value==='badges';document.querySelector('#ship-fittings').hidden=showBadges;badgePanel.hidden=!showBadges;refreshBadges();acknowledgeBadges();});
 ...
 function acknowledgeBadges(){if(!badgePanel.hidden&&!document.querySelector('#ship-accessories').hidden)badges.markSeen(badges.views().filter(b=>b.owned).map(b=>b.id));}
 ...
 function refreshBadges(){
  ...
  if(!selectedBadge){badgeDetail.textContent='徽章来自航行中的相遇，不消耗海里。';return;}
 ...
 function finishName(save,focus=false){
  if(nameForm.hidden)return true;
  if(save){const name=shipName(nameInput.value);if(!name){feedback('给小船取一个名字吧。');nameInput.focus();return false;}if(name!==customization.data.name)customization.update({name});}
  nameInput.value=customization.data.name;nameForm.hidden=true;nameRow.hidden=false;document.querySelector('#ship-title-name').textContent=customization.data.name;journalView.refreshCover();if(focus)nameEdit.focus();return true;
 }
 nameEdit.addEventListener('click',()=>{mileageOpen(false);nameInput.value=customization.data.name;nameRow.hidden=true;nameForm.hidden=false;nameInput.focus();nameInput.select();});
 nameForm.addEventListener('submit',e=>{e.preventDefault();finishName(true,true);});
 document.addEventListener('pointerdown',e=>{if(phase==='sailing')return;if(!nameForm.contains(e.target)&&!nameEdit.contains(e.target))finishName(true);if(!e.target.closest('.mileage-block'))mileageOpen(false);},true);
 function refresh(){
 ...
 document.addEventListener('keydown',e=>{
  ...
  if(!nameForm.hidden&&e.key==='Escape'){e.preventDefault();e.stopImmediatePropagation();finishName(false,true);return;}
  if(balance.getAttribute('aria-expanded')==='true'&&e.key==='Escape'){e.preventDefault();e.stopImmediatePropagation();mileageOpen(false);balance.focus();return;}
